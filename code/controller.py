...
def action():
    ...
    if choice_1 == 'See':
        ...
        if choice_2 == 'Single-trade':
            instrument = 'XPD_USD' if not interactive else questionary.select(
                'Select instrument to trade:', choices=instrument_name, default='XPD_USD'
            ).ask()
            trade_amount = 90.0 if not interactive else float(input('Enter trade amount: '))
            strategy_name = 'ORB' if not interactive else questionary.select(
                'Choose strategy:', choices=['ORB', 'VWAP'], default='ORB'
            ).ask()
        ...
        if choice_3 == 'My details':
            ...
            _ = 'Back' if not interactive else questionary.select('Choose account:', choices=accounts).ask()
            time.sleep(2)
            action()
        else:
            action()
